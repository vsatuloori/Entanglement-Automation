...
class HighDim:
    ...
    def __init__(self, file="CWEntanglement/CW_HighDimExptSettings_20250704.yaml", params=None, 
                 timetagger=None, interferometer=None, powermeter=None, OSW=None, 
                 yokogawa=None):
        ...
        if (file):
            ...
            self.TT = TT(params=yaml_data["TimeTagger"])

            # Initialize Interferometer
            self.interferometer = Interferometer(data=yaml_data)
    ...
    def initDevices(self):
        ...
        if (not self.yoko):
            try:
                self.yoko = yoko.yoAQ2212_frame_controller(ipAddress="10.7.0.30", port=50000, timeout=5)
                print(f"Yokoframe is connected successfully: {self.yoko}")
            except Exception as e:
    ...
    def parse_channel(self, ch_key):
        ...
        if isinstance(ch_key, str) and ch_key.startswith('Ch'):
            channel_info = self.TT.ChDict[ch_key]
...
if __name__ == "__main__":
    ...
    try: 
        ...
        from SHGScanTEC_v2 import SHG  # Import SHG class only if needed
